...
from stable_nalu.layer import RealNPULayer, NPULayer
from stable_nalu.layer import ReciprocalNMULayer, ConcatReciprocalNMUSignRetrievalLayer
...
parser.add_argument('--nalm-name',
                    action='store',
                    default='',
                    type=str,
                    help='Name of nalm model to use. Only need to set for division')
...
parser.add_argument('--only-nalm-cgn',
                    action='store_true',
                    default=False,
                    help='Only clips the grad of the nalms params gradient.')
...
parser.add_argument('--weight-decay',
                    ...
                    default=0,
                    type=float,
                    help='Weight decay to optimizer (i.e. l2 decay).')
parser.add_argument('--use-tau',
                    action='store_true',
                    default=False,
                    help='Use temp scaling for softmax. Decays over epochs automatically.')
parser.add_argument('--use-argmax',
                    action='store_true',
                    default=False,
                    help='Use argmax on outputs of the img2labels net..')
parser.add_argument('--dummy-writer',
                    action='store_true',
                    default=False,
                    help='Using Dummywriter means no tensorboard logging.')

round_n_dp = lambda arr, n_digits: (arr * 10 ** n_digits).round() / (10 ** n_digits)
...
def create_tb_writer(args, fold_idx, use_dummy=False):
    if use_dummy:
        ...
        return writer.SummaryWriter(
            ...
            f'_s{args.seed}'
            f'_mod-{args.nalm_name}',
            remove_existing_data=args.remove_existing_data
        )


def linear_temp_decay_params(temp_start=1, temp_end=1e-3, temp_decay_start_ep=0, temp_decay_end_ep=1000):
    # calculate the gradient and constant params for the linear decay i.e., the m and c in y= mx + c
    m = (temp_start - temp_end) / (temp_decay_start_ep - temp_decay_end_ep)
    c = temp_start - (m * temp_decay_start_ep)
    return m, c


def calculate_linear_temp(ep, m, c, temp_start=1, temp_end=1e-3):
    # y = mx+c with clipping to max/min tau before/after the e_start/e_end is reached. Let x = current epoch.
    y = m * ep + c
    y = min(temp_start, max(temp_end, y))
    return y
...
class Img2LabelsIndepImgClf(ExtendedTorchModule):
    ...
    """
    def __init__(self, device, no_zero=False, use_argmax=False, **kwargs):
        super(Img2LabelsIndepImgClf, self).__init__('cnn_1_digit_clf', **kwargs)
        self.no_zero = no_zero   # if 0's are included in the labels
        ...
        self.fc2 = nn.Linear(128, 9) if self.no_zero else nn.Linear(128, 10) # division doesn't use 0
        self.device = device
    ...
    def forward(self, x, tau=1):
        ...
        x = self.fc2(x)             # [2*B, 10]
        # [2B,10] -> [2B,1] -> [B,2]
        output = F.softmax(x/tau, dim=1)  # [2*B,10]

        if not self.training and self.use_argmax:
            ...
            # softargmax
            low_bound = 1 if self.no_zero else 0
            digits = torch.arange(low_bound, 10, 1).type(torch.FloatTensor).to(self.device)  # [10] numbers to index from
...
class MLPRegression(ExtendedTorchModule):
    """
    MLP to do regression part of the network, i.e., converting the predicted image labels to the final output.
    """
    def __init__(self, in_features=2, out_features=1, hid_features=256,  **kwargs):
        super(MLPRegression, self).__init__('mlp_regression', **kwargs)
        self.i2h = nn.Linear(in_features, hid_features)
        self.h2h = nn.Linear(hid_features, hid_features)
        self.h2o = nn.Linear(hid_features, out_features)

    def forward(self, x,):
        x = self.i2h(x)     # [B,2]   -> [B,H]
        x = F.relu(x)
        x = self.h2h(x)     # [B,H] -> [B,H]
        x = F.relu(x)
        x = self.h2o(x)     # [B,H] -> [B,1]
        return x
    ...
    def __init__(self, in_features, out_features, learn_labels2out, operation, **kwargs):
        ...
        if learn_labels2out:
            if self.operation == 'add':
                ...
                self.mlp = MLPRegression()
                # raise Exception("Labels2Out: learn_labels2out not supported with div!")
...
class Net(ExtendedTorchModule):
    def __init__(self, img2label_in=2, img2label_out=1, use_nalm=False, learn_labels2out=False, operation=None,
                 writer=None, device=None, use_argmax=False, **kwags):
        ...
        self.img2label = Img2LabelsIndepImgClf(device=device, no_zero=self.operation == 'div', use_argmax=use_argmax)
        ...
        if use_nalm:
            if operation == 'add':
                ...
                if kwags['nalm_name'] == 'realnpu-mod':
                    self.labels2Out = RealNPULayer(img2label_in, img2label_out, nac_oob='clip', writer=self.writer,
                                                   realnpu_reg_type='W',
                                                   npu_clip='wg', npu_Wr_init='xavier-uniform-constrained', **kwags)
                elif kwags['nalm_name'] == 'nru':
                    self.labels2Out = ReciprocalNMULayer(img2label_in, img2label_out, nac_oob='clip',
                                                         regualizer_shape='linear', writer=self.writer,
                                                         nru_div_mode='div',
                                                         trash_cell=False, **kwags)
                elif kwags['nalm_name'] == 'nmru-sign':
                    self.labels2Out = ConcatReciprocalNMUSignRetrievalLayer(img2label_in, img2label_out, nac_oob='clip',
                                                                            regualizer_shape='linear',
                                                                            writer=self.writer, use_noise=False,
                                                                            **kwags)
                else:
                    raise Exception("Wrong division nalm_name given!")
        else:
...
class TwoDigitExperiment:
    ...
    def get_label2out_weight(self, model, idx, is_nalm, learn_last_layer, nalm_name=None):
        if is_nalm:
            if nalm_name == 'nmru-sign':
                # return reciprocal weight
                if idx == 1:
                    return model.labels2Out.W.view(-1)[2].item()
                # return numerator (mul) weight
                else:
                    return model.labels2Out.W.view(-1)[idx].item()
            elif nalm_name == 'realnpu-mod':
                return model.labels2Out.get_label2out_weight(idx)
            else:
                return model.labels2Out.W.view(-1)[idx].item()
    ...
    def calc_reg_loss(self, model, w_scale, args):
        ...
        reg_loss = regualizers['W'] * w_scale * args.regualizer + \
                   args.regualizer_npu_w * w_scale * regualizers['W-NPU'] + \
                   args.regualizer_gate * w_scale * regualizers['g-NPU']
        return reg_loss
        ...
        for train_set_pair in set_pairs:
            for _ in range(args.samples_per_permutation):
                ...
                if args.operation == 'add':
                    ...
                    y.append(target_zi.to(float) / target_zj.to(float))
                else:
    ...
    def eval_dataloader(self, model, device, dataloader, summary_writer, state):
        ...
        correct_output_rounded_4dp = 0
        correct_output_rounded_5dp = 0
        ...
        with torch.no_grad():
            for data, target, img_label_targets in dataloader:
                ...
                # sys.exit(0)
                ###############################################################################################
                output, img_labels_output = model(data, tau=self._tau)
                ...
                correct_output_rounded_4dp += round_n_dp(output, 4).eq(round_n_dp(target.view_as(output), 4)).sum().item()
                correct_output_rounded_5dp += round_n_dp(output, 5).eq(round_n_dp(target.view_as(output), 5)).sum().item()
        ...
        acc_output_rounded_4dp = 100. * correct_output_rounded_4dp / len(dataloader.dataset)
        acc_output_rounded_5dp = 100. * correct_output_rounded_5dp / len(dataloader.dataset)
        ...
        print('{}: {:.5f}, acc: {}/{} ({:.2f}%), img1: {:.5f} ({:.2f}%), img2: {:.5f} ({:.2f}%)\t '
              'Rounded: acc: {}/{} ({:.2f}%), img1:({:.2f}%), img2: ({:.2f}%)'
              '\tRounded: out acc: 4dp: {:.3f}%, 5dp: {:.3f}%'.format(
            ...
            acc_label_1_rounded,
            acc_label_2_rounded,
            acc_output_rounded_4dp,
            acc_output_rounded_5dp,
        ))
        ...
        summary_writer.add_scalar(f'metric/{state}/output_rounded_4dp/acc', acc_output_rounded_4dp)
        summary_writer.add_scalar(f'metric/{state}/output_rounded_5dp/acc', acc_output_rounded_5dp)
    ...
    def epoch_step(self, model, train_loader, args, optimizer, epoch, w_scale, summary_writer, test_loader):
        ...
        """
        self._tau = calculate_linear_temp(epoch, m=self.temp_decay_grad,
                                          c=self.temp_decay_constant) if args.use_tau else 1
        ...
        for batch_idx, (data, target, img_label_targets) in enumerate(train_loader):
            ...
            if epoch % args.log_interval == 0 and batch_idx == 0:
                ...
                if args.learn_labels2out:
                    if not args.operation == 'div' or not args.learn_labels2out:
            ...
            if total_loss.requires_grad:
                ...
                if args.clip_grad_norm != None:
                    # only grad norm clip the nalm params
                    if args.only_nalm_cgn:
                        torch.nn.utils.clip_grad_norm_(model.labels2Out.parameters(), args.clip_grad_norm)
                    else:
    ...
    def main(self):
        ...
        self.temp_decay_grad, self.temp_decay_constant = linear_temp_decay_params(temp_decay_end_ep=args.max_epochs)
        print(f"Temp decay -- start ep: {0}; end ep: {args.max_epochs}; "
              f"grad: {self.temp_decay_grad}; constant: {self.temp_decay_constant}\n")

        # sanity checks
        if args.only_nalm_cgn:
            assert args.use_nalm, "Cannot use only_nalm_cgn is use_nalm is not activated!"
            assert args.clip_grad_norm, "To use only_nalm_cgn, clip_grad_norm must be set."
        ...
        # args.dummy_writer = True
        use_dummy_writer = args.dummy_writer  # dummywriter won't save tensorboard files
        ...
        # Don't have 0's for division
        if args.operation == 'div':
            unique_pairs = [str(x) + str(y) for x in range(1, 10) for y in range(1, 10)]
        else:
        ...
        print("Train set pairs", train_set_pairs)
        print("Test set pairs", test_set_pairs)

        # Sanity checks
        if args.operation == 'div':
            assert (len(test_set_pairs) in [8, 9])      # seed 0 has an extra test value
            assert (len(train_set_pairs) in [73, 72])
        else:
        ...
        optimizer = torch.optim.Adam([
            {'params': model.img2label.parameters()},
            {'params': model.labels2Out.parameters(), 'weight_decay': args.weight_decay}
        ...
        if args.load_checkpoint:
            load_filename = None # TODO
            resume_epoch = writer.load_model(load_filename, model, optimizer)  # add one for next epoch
            if resume_epoch > args.max_epochs:
                raise ValueError(
                    f'{args.max_epochs} must be larger than or equal to the loaded models resume epoch {resume_epoch}')
            # if resume_epoch != 0:
        ...
        ###########################################################################################################
        # Train/test loop
        for epoch in range(resume_epoch, args.max_epochs + 1):
            print(f"Epoch {epoch}")
            w_scale = max(0, min(1, (
